...
async def spawned_goblin(gsm: GameStateManager, ai_test_map):
    ...
    try:
        await gsm.despawn_entity(instance_id, death_tick=0, respawn_delay_seconds=0)
    except Exception:
    ...
    async def test_idle_entity_transitions_to_wander(
        ...
        with patch("server.src.services.ai_service.settings") as mock_settings:
            ...
            with patch("server.src.services.ai_service.PlayerService.get_players_on_map", new_callable=AsyncMock) as mock_players:
                ...
                await AIService.process_entities(gsm, map_id, current_tick=100)
...
class TestWanderStateIntegration:
    ...
    async def test_wandering_entity_moves(
        ...
        original_entity = await gsm.get_entity_instance(instance_id)
...
class TestReturningIntegration:
    ...
    async def test_returning_entity_moves_to_spawn(
        ...
        await gsm.update_entity_position(instance_id, 55, 50)
    ...
    async def test_returning_entity_heals_at_spawn(
        ...
        await gsm.update_entity_hp(instance_id, 50)
        await gsm.set_entity_state(instance_id, EntityState.RETURNING)
...
class TestFullStateMachineCycle:
    ...
    async def test_full_combat_cycle(
        ...
        with patch("server.src.services.ai_service.settings") as mock_settings:
            ...
            with patch("server.src.services.ai_service.PlayerService.get_players_on_map", new_callable=AsyncMock) as mock_players:
                mock_players.return_value = []  # Player left
                await AIService.process_entities(gsm, map_id, current_tick=110)
            ...
            await AIService.process_entities(gsm, map_id, current_tick=200)
    ...
    async def test_wander_cycle(
        ...
        with patch("server.src.services.ai_service.settings") as mock_settings:
            ...
            with patch("server.src.services.ai_service.PlayerService.get_players_on_map", new_callable=AsyncMock) as mock_players:
                ...
                await gsm.update_entity_position(instance_id, wander_target[0], wander_target[1])
                await AIService.process_entities(gsm, map_id, current_tick=200)
...
class TestAIEdgeCases:
    ...
    async def test_process_entities_handles_missing_entity_def(
        ...
        # Spawn entity with unknown name - should be skipped without error
        instance_id = await gsm.spawn_entity_instance(
            entity_name="UNKNOWN_ENTITY_TYPE_12345",
            entity_type=EntityType.MONSTER,
            map_id=map_id,
            x=50, y=50,
    ...
    async def test_dead_entities_skipped(
        ...
        await gsm.set_entity_state(instance_id, EntityState.DEAD)
