...
class TestConcurrencyIntegration:
    ...
    async def test_concurrent_inventory_operations_real_gsm(self, session, game_state_managers):
        ...
        async def concurrent_inventory_ops(operation_id: int):
            ...
            for i in range(10):
                ...
                inventory = await inv_mgr.get_inventory(player_id)
                ...
                if i % 3 == 0:
                    ...
                    inventory_after_delete = await inv_mgr.get_inventory(player_id)
                    results.append({
                        'slot': slot,
        ...
        final_inventory = await inv_mgr.get_inventory(player_id)
    ...
    async def test_concurrent_equipment_operations_real_gsm(self, session, game_state_managers):
        ...
        await player_mgr.register_online_player(player_id, "testplayer2")
        ...
        async def concurrent_equipment_ops(operation_id: int):
            ...
            for i in range(20):
                ...
                if i % 4 == 0:
                    ...
                    equipment_after_delete = await equip_mgr.get_equipment(player_id)
                    results.append({
                        'slot': slot_name,
    ...
    async def test_concurrent_player_hp_operations_real_gsm(self, session, game_state_managers):
        ...
        # Ensure the player is considered online
        await player_mgr.register_online_player(player_id, "testplayer3")
        ...
        final_hp = await player_mgr.get_player_hp(player_id)
    ...
    async def test_cross_operation_isolation(self, session, game_state_managers):
        ...
        await player_mgr.register_online_player(player1.id)
        await player_mgr.register_online_player(player2.id)
        ...
        async def mixed_operations_player1():
            ...
            for i in range(20):
                # Inventory operation
                await inv_mgr.set_inventory_slot(player1.id, i % 28, i + 1, 1, 0.9)
                
                # Equipment operation
                if i % 5 == 0:
                    await equip_mgr.set_equipment_slot(player1.id, EquipmentSlot.WEAPON.value, i + 100, 1, 0.8)
                
                # HP operation
                if i % 3 == 0:
                    await player_mgr.set_player_hp(player1.id, 50 + i, 100 + i)
        ...
        async def mixed_operations_player2():
            ...
            for i in range(20):
                ...
                await inv_mgr.set_inventory_slot(player2.id, i % 28, i + 200, 2, 0.7)
                
                # Equipment operation (different items)
                if i % 4 == 0:
                    await equip_mgr.set_equipment_slot(player2.id, EquipmentSlot.HEAD.value, i + 300, 1, 0.6)
                
                # HP operation (different values)
                if i % 2 == 0:
                    await player_mgr.set_player_hp(player2.id, 30 + i, 80 + i)
        ...
        p1_inventory = await inv_mgr.get_inventory(player1.id)
        p2_inventory = await inv_mgr.get_inventory(player2.id)
        ...
        p1_equipment = await equip_mgr.get_equipment(player1.id)
        p2_equipment = await equip_mgr.get_equipment(player2.id)
