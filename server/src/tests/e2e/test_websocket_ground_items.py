...
class TestGroundItemsInGameStateUpdate:
    ...
    async def test_drop_item_creates_ground_item(self, session, gsm):
        ...
        # Verify item was created through ground item manager
        from server.src.services.game_state import get_ground_item_manager
        ground_item_mgr = get_ground_item_manager()
        ground_items = await ground_item_mgr.get_ground_items_on_map("samplemap")
        our_item = next((gi for gi in ground_items if gi["ground_item_id"] == result.ground_item_id), None)
