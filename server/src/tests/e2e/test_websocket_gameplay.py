...
class TestVisibilitySystem:
    ...
    @pytest.mark.asyncio
    async def test_visibility_diff_added(self):
        ...
        # First update with empty state - everything is "new"
        visible_entities = {
            "player_2": {"id": "player_2", "x": 10, "y": 10, "username": "player2"},
            "entity_123": {"id": "entity_123", "x": 15, "y": 15, "type": "npc"},
        }
        
        diff = await visibility.update_player_visible_entities(player_id, visible_entities)
        
        # All entities should be in added list on first update
        assert len(diff["added"]) == 2
        assert len(diff["updated"]) == 0
        assert len(diff["removed"]) == 0
        
        # Verify entities are in the added list
        added_ids = {e["id"] for e in diff["added"]}
        assert "player_2" in added_ids
        assert "entity_123" in added_ids

    @pytest.mark.asyncio
    async def test_visibility_diff_removed(self):
        ...
        await visibility.update_player_visible_entities(player_id, visible_entities)
        
        # Second update - one entity removed
        visible_entities = {
            "player_2": {"id": "player_2", "x": 10, "y": 10, "username": "player2"},
        }
        ...
        # Entity 123 should be in removed list
        assert len(diff["added"]) == 0
        assert len(diff["updated"]) == 0
        assert len(diff["removed"]) == 1
        assert diff["removed"][0]["id"] == "entity_123"

    @pytest.mark.asyncio
    async def test_visibility_diff_moved(self):
        ...
        visible_entities = {
            "player_2": {"id": "player_2", "x": 11, "y": 10, "username": "player2"},
        }
        
        diff = await visibility.update_player_visible_entities(player_id, visible_entities)
        
        # Player 2 should be in updated list
        assert len(diff["added"]) == 0
        assert len(diff["updated"]) == 1
        assert len(diff["removed"]) == 0
        assert diff["updated"][0]["id"] == "player_2"
        assert diff["updated"][0]["x"] == 11

    @pytest.mark.asyncio
    async def test_visibility_diff_no_change(self):
        """Entities that didn't change should not be in any list."""
        from server.src.services.visibility_service import VisibilityService
        ...
        # No changes - all lists should be empty
        assert len(diff["added"]) == 0
        assert len(diff["updated"]) == 0
        assert len(diff["removed"]) == 0
