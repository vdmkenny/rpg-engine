...
class SkillData(BaseModel):
    ...
    level: int = Field(..., ge=1, le=99, description="Current skill level")
    xp: int = Field(..., ge=0, description="Total experience points")
    xp_for_current: int = Field(..., ge=0, description="XP required to reach current level")
    xp_for_next: int = Field(..., ge=0, description="XP required to reach next level")
    xp_to_next: int = Field(..., ge=0, description="XP remaining until next level")
...
class XPGain(BaseModel):
    ...
    xp: int = Field(..., ge=0, description="Current total XP in the skill")
